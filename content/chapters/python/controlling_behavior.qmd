...
<!-- #### Exercise
Say you have two values represented by the variables `x` and `y`. Now you want to assign the largest of those two values to a third value `z`. Can you see that the two pieces of code below are essentially two ways of expressing the same thing? (You must define `x` and `y` yourself and print `z` to see its value.) This is tricky, but it trains both variables, assignment, operators, precedence, substitution, reduction, logic, and control flow. Make sure you are aware of how.
