...
4. The return statement ends the function. After the `return` keyword, the expression reduces to a *value* that the function returns.
...
3. You can define functions with any number of *parameters* if you use the same number of *arguments* when you call the function.
...
Try (possibly strange) variations of the code like the ones below better to understand the contribution of each line of code. What is the difference between return and print?
What happens when Python gets to a `return` statement in a function? What happens when the function does not have a `return` statement?
...
Can you complete this exercise using the `is_even` you defined in @sec-is_even? How? Why is that a good idea?
