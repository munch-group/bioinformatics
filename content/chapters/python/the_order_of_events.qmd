# The order of events {#sec-precedence}
...
::: {#important-oath3 .callout-important }
...
Compare the final value of `x` to that in @sec-adding_one. Can you see what `+=` is a shorthand for? Nifty, right?
...
In @sec-type_coercion, you investigated what types of values the `*` operator produces. Redo that exercise with the operators: `+`, `-`, `/`, `**`, `//`, and `%`. What are the rules for what is returned if both values are integers, one value is a float, or both values are floats?
