...
## Introduction: The Need for Database Searching  {.unnumbered}
...
## The Computational Challenge  {.unnumbered}

### Scale and Complexity  {.unnumbered}
...
### Evolution and Sequence Similarity  {.unnumbered}
...
## BLAST: The Basic Local Alignment Search Tool  {.unnumbered}

### Historical Development and Impact  {.unnumbered}
...
### The Original BLAST Algorithm  {.unnumbered}
...
```{python}
#| echo: false
#| label: fig-blast-workflow
#| fig-cap: "BLAST algorithm workflow showing the three main phases: seeding (word matching), extension (ungapped then gapped), and evaluation (statistical significance). The seed-and-extend strategy dramatically reduces search space while maintaining sensitivity."
#| fig-width: 7
#| fig-format: svg
...
boxes = {
    'query': {'pos': (0.5, 0.95), 'text': 'Query\nSequence', 'color': '#e1f5ff'},
    'word': {'pos': (0.5, 0.85), 'text': 'Generate word\nneighborhood\n(length W, threshold T)', 'color': '#fff8e1'},
    'scan': {'pos': (0.5, 0.72), 'text': 'Scan database for\nexact word matches\n(finite state machine)', 'color': '#fff8e1'},
    'match': {'pos': (0.5, 0.59), 'text': 'Word match\nfound?', 'color': '#ffe1e1', 'shape': 'diamond'},
    'ungapped': {'pos': (0.5, 0.46), 'text': 'Ungapped extension\n(dropout parameter X)', 'color': '#fff8e1'},
    'score': {'pos': (0.5, 0.33), 'text': 'Score > S\nthreshold?', 'color': '#ffe1e1', 'shape': 'diamond'},
    'gapped': {'pos': (0.5, 0.20), 'text': 'Gapped extension\n(dynamic programming)', 'color': '#fff8e1'},
    'eval': {'pos': (0.5, 0.10), 'text': 'Calculate E-value\n(Karlin-Altschul)', 'color': '#fff8e1'},
    'evalue': {'pos': (0.85, 0.10), 'text': 'E-value <\ncutoff?', 'color': '#ffe1e1', 'shape': 'diamond'},
    'report': {'pos': (0.85, 0.20), 'text': 'Report HSP', 'color': '#e1ffe1'},
}
...
for key, box in boxes.items():
    ...
    if box.get('shape') == 'diamond':
        ...
        diamond = mpatches.Polygon([
            [x, y + 0.04], [x + 0.08, y], [x, y - 0.04], [x - 0.08, y]
        ], closed=True, facecolor=box['color'], edgecolor='black', linewidth=2)
        ...
        ax.add_patch(rect)
    ax.text(x, y, box['text'], ha='center', va='center', fontsize=9, weight='bold')

# Draw arrows
arrows = [
    ((0.5, 0.91), (0.5, 0.89)),  # query -> word
    ((0.5, 0.81), (0.5, 0.76)),  # word -> scan
    ((0.5, 0.68), (0.5, 0.63)),  # scan -> match
    ((0.5, 0.55), (0.5, 0.50)),  # match Yes -> ungapped
    ((0.5, 0.42), (0.5, 0.37)),  # ungapped -> score
    ((0.5, 0.29), (0.5, 0.24)),  # score Yes -> gapped
    ((0.5, 0.16), (0.5, 0.14)),  # gapped -> eval
    ((0.68, 0.10), (0.77, 0.10)),  # eval -> evalue
    ((0.85, 0.16), (0.85, 0.14)),  # evalue Yes -> report
    # No loops
    ((0.42, 0.59), (0.15, 0.59), (0.15, 0.72), (0.41, 0.72)),  # match No -> scan
    ((0.42, 0.33), (0.25, 0.33), (0.25, 0.72), (0.41, 0.72)),  # score No -> scan
    ((0.93, 0.10), (0.99, 0.10), (0.99, 0.72), (0.59, 0.72)),  # evalue No -> scan
]

for arrow in arrows[:9]:  # Straight arrows
    ax.annotate('', xy=arrow[1], xytext=arrow[0],
               arrowprops=dict(arrowstyle='->', lw=2, color='black'))

# Draw curved "No" arrows
for i, coords in enumerate([arrows[9], arrows[10], arrows[11]]):
    path = mpatches.FancyBboxPatch((0, 0), 1, 1)
    if len(coords) == 4:
        # Multi-segment path
        for j in range(len(coords) - 1):
            ax.plot([coords[j][0], coords[j+1][0]], [coords[j][1], coords[j+1][1]],
                   'k-', lw=2)
        ax.annotate('', xy=coords[-1], xytext=(coords[-2][0], coords[-2][1]),
                   arrowprops=dict(arrowstyle='->', lw=2, color='black'))
        # Add "No" label
        if i == 0:
            ax.text(0.20, 0.65, 'No', fontsize=8, style='italic', color='red')
        elif i == 1:
            ax.text(0.30, 0.50, 'No', fontsize=8, style='italic', color='red')
        elif i == 2:
            ax.text(0.95, 0.40, 'No', fontsize=8, style='italic', color='red')

# Add "Yes" labels
ax.text(0.52, 0.52, 'Yes', fontsize=8, style='italic', color='green')
ax.text(0.52, 0.26, 'Yes', fontsize=8, style='italic', color='green')
ax.text(0.87, 0.13, 'Yes', fontsize=8, style='italic', color='green')
...
### Scoring Local Alignments  {.unnumbered}
...
### Mathematical Foundation: Karlin-Altschul Statistics  {.unnumbered}
...
## BLAST Parameters and Their Effects  {.unnumbered}

### Word Size (W)  {.unnumbered}
...
### Substitution Matrices  {.unnumbered}
...
## E-values and Statistical Significance  {.unnumbered}

### Understanding E-values  {.unnumbered}
...
### Query Length Effects  {.unnumbered}
...
### Significance Thresholds  {.unnumbered}
...
### Composition-Based Statistics  {.unnumbered}
...
## BLAST Variants and Improvements  {.unnumbered}

### BLAST2 and the Two-Hit Algorithm  {.unnumbered}
...
### PSI-BLAST: Position-Specific Iterated BLAST  {.unnumbered}
...
### PHI-BLAST: Pattern-Hit Initiated BLAST  {.unnumbered}
...
### DELTA-BLAST: Domain Enhanced Lookup Time Accelerated BLAST  {.unnumbered}
...
## Specialized BLAST Programs  {.unnumbered}

### BLASTN: Nucleotide-Nucleotide Searches  {.unnumbered}
...
### BLASTX and TBLASTN: Translated Searches  {.unnumbered}
...
### TBLASTX: Six-Frame Translation  {.unnumbered}
...
## Advanced Applications and Considerations  {.unnumbered}

### Database Contamination and Quality  {.unnumbered}
...
### Taxonomic Restrictions and Organism-Specific Searches  {.unnumbered}
...
### Batch Searches and Automation  {.unnumbered}
...
### Cloud and Web Services  {.unnumbered}
...
## Limitations and Alternatives  {.unnumbered}

### Limitations of BLAST  {.unnumbered}
...
## Best Practices and Interpretation  {.unnumbered}

### Choosing Appropriate Parameters  {.unnumbered}
...
### Integration with Structure Prediction  {.unnumbered}
...
### Machine Learning Approaches  {.unnumbered}
...
### Scaling to Massive Databases  {.unnumbered}
...
### Metagenomics and Environmental Sequencing  {.unnumbered}
