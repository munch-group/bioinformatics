# Clustering of sequences {#sec-clustering}
...
## Which pair to cluster next  {.unnumbered}
...
## How to computes distances from the new cluster  {.unnumbered}
...
# Molecular clock {#sec-molecularclock}
...
## Example of wrong tree produced in absence of molecular clock  {.unnumbered}
...
The smallest distance is $d(A,D) = 1$. Therefore, we cluster A and D together to form a new cluster V = (A,D). The height of this merge in the dendrogram is $1/2 = 0.5$.

**Step 2: Computing Distances to Cluster V**
Panel (B) of @fig-upgma-example shows the reduced matrix after clustering A and D. We need to compute distances from the new cluster V to all remaining sequences using the UPGMA formula. Since both A and D are single sequences, $n_A = n_D = 1$:

$$d(V,B) = \frac{1 \times d(A,B) + 1 \times d(D,B)}{1 + 1} = \frac{6 + 6}{2} = 6$$

$$d(V,C) = \frac{1 \times d(A,C) + 1 \times d(D,C)}{1 + 1} = \frac{8 + 8}{2} = 8$$

$$d(V,E) = \frac{1 \times d(A,E) + 1 \times d(D,E)}{1 + 1} = \frac{2 + 2}{2} = 2$$

$$d(V,F) = \frac{1 \times d(A,F) + 1 \times d(D,F)}{1 + 1} = \frac{6 + 6}{2} = 6$$

The smallest distance in the new matrix is $d(V,E) = 2$. We cluster V and E to form W = ((A,D),E) at height $2/2 = 1$.

**Step 3: Computing Distances to Cluster W**
Panel (C) of @fig-upgma-example shows the matrix after forming cluster W = ((A,D),E). Since cluster V contained 2 sequences (A and D) and E is a single sequence, we use $n_V = 2$ and $n_E = 1$ in our calculations:

$$d(W,B) = \frac{2 \times d(V,B) + 1 \times d(E,B)}{2 + 1} = \frac{2 \times 6 + 1 \times 6}{3} = \frac{18}{3} = 6$$

$$d(W,C) = \frac{2 \times d(V,C) + 1 \times d(E,C)}{2 + 1} = \frac{2 \times 8 + 1 \times 8}{3} = \frac{24}{3} = 8$$

$$d(W,F) = \frac{2 \times d(V,F) + 1 \times d(E,F)}{2 + 1} = \frac{2 \times 6 + 1 \times 6}{3} = \frac{18}{3} = 6$$

The smallest distance is $d(B,F) = 4$. We cluster B and F to form X = (B,F) at height $4/2 = 2$.

**Step 4: Computing Distances Between Three Clusters**
Panel (D) of @fig-upgma-example shows the matrix with three clusters: W (containing A, D, E), X (containing B, F), and C. We compute the distance between W and X using the fact that B and F were both single sequences when merged:

$$d(W,X) = \frac{1 \times d(W,B) + 1 \times d(W,F)}{1 + 1} = \frac{6 + 6}{2} = 6$$

Similarly, for the distance between C and X:

$$d(C,X) = \frac{1 \times d(C,B) + 1 \times d(C,F)}{1 + 1} = \frac{8 + 8}{2} = 8$$

The smallest distance is $d(W,X) = 6$. We cluster W and X to form Y = (((A,D),E),(B,F)) at height $6/2 = 3$.

**Step 5: Final Clustering**
Panel (E) of @fig-upgma-example shows the final step with only two clusters remaining: Y and C. The distance between them is 8, so they merge at height $8/2 = 4$.

The complete UPGMA tree shows the hierarchical clustering with merge heights:
- A and D merge at height 0.5
- (A,D) and E merge at height 1
- B and F merge at height 2
- ((A,D),E) and (B,F) merge at height 3
- (((A,D),E),(B,F)) and C merge at height 4

This example illustrates several key features of UPGMA: the algorithm always selects the minimum distance at each step, the merge height equals half the distance (assuming a molecular clock), and the distance calculations use weighted averages based on cluster sizes to maintain the unweighted nature of the method.

## Neighbor-joining {#sec-upgma .unnumbered}
...
### Additive distances / Non-recombining sequence ancestry  {.unnumbered}
...
### Minimal evolution principle  {.unnumbered}
...
# Bootstrap Analysis in Phylogenetic Reconstruction  {#sec-bootstrap}
...
## Statistical Principles of Bootstrapping  {.unnumbered}
...
## Bootstrap Assumptions and Requirements  {.unnumbered}
...
## What Bootstrap Values Quantify  {.unnumbered}
...
## When Bootstrap Analysis is Appropriate  {.unnumbered}
...
## Computational Implementation  {.unnumbered}
...
## Interpreting Bootstrap Results  {.unnumbered}
...
## Advanced Bootstrap Methods  {.unnumbered}
