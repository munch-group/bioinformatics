...
## Introduction: Why Model DNA Evolution?  {.unnumbered}
...
## The Fundamental Challenge: Multiple Substitutions  {.unnumbered}
...
## A model of DNA evolution  {.unnumbered}
...
## The Jukes-Cantor Model: Simplicity and Symmetry  {.unnumbered}
...
### Model Assumptions and Structure  {.unnumbered}

The JC69 model makes several simplifying assumptions:
1. All nucleotides (A, C, G, T) have equal equilibrium frequencies (πA = πC = πG = πT = 0.25)
2. All substitutions occur at the same rate α
...
### Deriving Transition Probabilities  {.unnumbered}
...
### Worked Example: Computing Transition Probabilities  {.unnumbered}
...
### Two-Sequence Divergence  {.unnumbered}
...
### Distance Correction Formula  {.unnumbered}
...
### Numerical Example: Distance Correction  {.unnumbered}
...
### Saturation and Model Limitations  {.unnumbered}
...
### Properties and Insights  {.unnumbered}
...
3. **Reversibility**: P(i→j in time t) × πᵢ = P(j→i in time t) × πⱼ
...
## The Kimura Two-Parameter Model: Transitions and Transversions  {.unnumbered}
...
## The Felsenstein 81 Model: Unequal Base Frequencies  {.unnumbered}
...
## The HKY85 Model: Combining Biological Realism  {.unnumbered}
...
## The General Time Reversible Model: Maximum Flexibility  {.unnumbered}
...
- JC69: all rᵢⱼ = 1, all πᵢ = 0.25
- K80: all πᵢ = 0.25, transitions share one rate, transversions another
- F81: all rᵢⱼ = 1, πᵢ estimated from data
- HKY85: transitions share one rate, transversions another, πᵢ estimated
...
## Rate Heterogeneity: The Gamma Distribution  {.unnumbered}
...
The shape parameter α determines the extent of rate variation:
- α < 1: L-shaped distribution with many slowly evolving sites
- α = 1: exponential distribution
- α > 1: bell-shaped distribution
- α → ∞: no rate variation (all sites evolve at the same rate)
...
## Invariant Sites: Accounting for Functional Constraints  {.unnumbered}
...
## Codon Models: The Genetic Code's Influence  {.unnumbered}
...
where ω = dN/dS represents the ratio of non-synonymous to synonymous substitution rates. This ratio provides information about selection:
- ω < 1: purifying selection (most amino acid changes are deleterious)
- ω = 1: neutral evolution
- ω > 1: positive selection (amino acid changes are advantageous)
...
## Context-Dependent Evolution: Beyond Independence  {.unnumbered}
...
- CpG effects: elevated C→T rates in CpG contexts
...
## Model Selection: Choosing Appropriate Complexity  {.unnumbered}
...
## Practical Applications: From Theory to Practice  {.unnumbered}
